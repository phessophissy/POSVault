...
describe("SIP-010 token compliance test suite — scenario 5", () => {
  it("verifies vault state for scenario 5", () => {
  ...
  it("checks deposit for scenario 5", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(500000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 5", () => {
