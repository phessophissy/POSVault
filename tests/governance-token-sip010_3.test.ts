...
describe("SIP-010 token compliance test suite — scenario 3", () => {
  it("verifies vault state for scenario 3", () => {
  ...
  it("checks deposit for scenario 3", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(300000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 3", () => {
