...
describe("SIP-010 token compliance test suite — scenario 7", () => {
  it("verifies vault state for scenario 7", () => {
  ...
  it("checks deposit for scenario 7", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(700000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 7", () => {
