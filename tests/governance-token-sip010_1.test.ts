...
describe("SIP-010 token compliance test suite — scenario 1", () => {
  it("verifies vault state for scenario 1", () => {
  ...
  it("checks deposit for scenario 1", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(100000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 1", () => {
