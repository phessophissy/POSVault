...
describe("SIP-010 token compliance test suite — scenario 8", () => {
  it("verifies vault state for scenario 8", () => {
  ...
  it("checks deposit for scenario 8", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(800000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 8", () => {
