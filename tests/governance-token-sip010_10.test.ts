...
describe("SIP-010 token compliance test suite — scenario 10", () => {
  it("verifies vault state for scenario 10", () => {
  ...
  it("checks deposit for scenario 10", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(1000000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 10", () => {
