...
describe("SIP-010 token compliance test suite — scenario 4", () => {
  it("verifies vault state for scenario 4", () => {
  ...
  it("checks deposit for scenario 4", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(400000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 4", () => {
