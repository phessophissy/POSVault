...
describe("SIP-010 token compliance test suite — scenario 6", () => {
  it("verifies vault state for scenario 6", () => {
  ...
  it("checks deposit for scenario 6", () => {
    const r = simnet.callPublicFn(vault, "deposit", [Cl.uint(600000)], user1);
    expect(r.result).toHaveClarityType(ClarityType.ResponseOk);
  });

  it("validates token info for scenario 6", () => {
